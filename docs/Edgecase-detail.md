...
**Tình huống**
Owner gõ và tạo ra ba event theo thứ tự: v1 (`d`), v2 (`de`), v3 (`def`). Do mạng không ổn định, Viewer nhận được theo thứ tự v3, v1, v2.

**Vấn đề**
Nếu Viewer áp dụng event theo thứ tự nhận được, màn hình sẽ hiển thị `def` → `d` → `de` — code bị lộn xộn, không phản ánh đúng trạng thái hiện tại của Owner.
...
**Vấn đề**
Điều này có thể xảy ra do network drop thầm lặng, một event bị mất mà không ai biết. Viewer vẫn nghĩ mình đang xem đúng code của Owner nhưng thực ra đã lệch.
...
**Tình huống**
Viewer bị mất mạng trong 30 giây. Trong thời gian đó Owner vẫn gõ code bình thường. Khi Viewer kết nối lại, có thể có hàng chục event bị bỏ lỡ.
...
**Kiểm tra**
Ngắt kết nối Viewer, để Owner gõ thêm nhiều nội dung, sau đó reconnect. Xác nhận Viewer hiển thị đúng code hiện tại của Owner.
...
**Tình huống**
Kẻ tấn công lấy được một JWT hợp lệ, tự sửa payload để nâng role từ `USER` lên `ADMIN`, rồi dùng token đã sửa để gọi API.
...
**Vấn đề**
Người dùng có thể bypass frontend bằng cách gửi WebSocket message thủ công qua DevTools hoặc tool khác. Nếu backend không kiểm tra, code của Owner bị sửa mà Owner không hay biết — ảnh hưởng trực tiếp đến tính toàn vẹn của buổi phỏng vấn.

**Giải pháp**
Khi nhận event `code_changed`, backend kiểm tra xem người gửi có phải là Owner của session đó không — bằng cách so sánh `user_id` từ JWT với `owner_id` trong bản ghi session trên database. Nếu không khớp, event bị bỏ qua hoàn toàn và hành vi này được ghi vào audit log.
...
**Rủi ro là gì**
Viewer nhìn thấy code không đúng với những gì Owner đang viết — do event đến sai thứ tự, bị gửi trùng, hoặc một số event bị mất thầm lặng. Trong bối cảnh phỏng vấn, điều này khiến interviewer đánh giá sai năng lực ứng viên.
...
**Cách giảm thiểu**
Hai lớp bảo vệ: API response luôn mask `input` và `expected_output` của hidden test case thành `[hidden]` trước khi gửi cho User. Việc so sánh output diễn ra hoàn toàn phía server — User không bao giờ nhận được `expected_output` thực tế trong kết quả submission, kể cả khi test đó Sai. Hidden test case cũng không được log ở bất kỳ level nào.
...
**Cách giảm thiểu**
Backend kiểm tra quyền ở mỗi WebSocket event — không tin vào giao diện frontend. Mọi attempt sửa code từ người không phải Owner của session đều bị từ chối và ghi log.
...
- [ ] Session auto-close chỉ xảy ra khi quá 5 phút idle và owner không còn WebSocket connection
