...
|---------|----------------|-------|
| Source code của User/Owner | HIGH | Code bài thi, thuật toán cá nhân |
...
| **Malicious User** | Submit code độc, access hidden tests, abuse join flow | Gian lận bài thi, leo thang quyền |
| **Malicious Viewer** | Cố gắng edit code trong session | Phá hoại bài thi của Owner |
...
| **Elevation of Privilege** | User tự đổi role thành Admin | Role chỉ được set bởi Admin, verify từ DB mỗi request |
...
- User chỉ xem được submission của chính mình
- Session owner chỉ close/share/kick participant trong session của chính mình

### 4.3 RBAC Matrix (Security-Focused)

| Endpoint / Event | ADMIN | USER | Ghi chú |
|------------------|-------|------|---------|
| `POST /submissions/run` | ✅ | ✅ | Session viewer không chạy code |
...
| WebSocket `code_changed` | ✅ | ⚠️ owner of session | Verify session ownership |
| `POST /api/sessions/:id/sharing/open` | ✅ | ⚠️ owner only | Toggle sharing and issue join code |
| `POST /api/sessions/:id/participants/:user_id/kick` | ✅ | ⚠️ owner only | Remove viewer from session |
...
2. Load session từ DB, kiểm tra `session.owner_id === user_id`
...
| Hidden test case expected output | HIGH | DB, không log, không trả về non-admin user |
...
- [ ] User xem submission của user khác → `403`
- [ ] User gọi `POST /admin/questions` → `403`
- [ ] User xem hidden test case expected output → không có trong response
...
**Data Leakage:**
- [ ] API response cho User không chứa `expected_output` của hidden test case
